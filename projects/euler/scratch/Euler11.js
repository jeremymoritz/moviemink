...
function oldGreatestProductFromGrid () {
	var n = 4,
		tempAcrossProduct = 1,
		tempDownProduct = 1,
		tempDiagRightProduct = 1,
	...
	for (var i = 0; i < initialArray.length; i++) {
		//acrossProduct:
		for (var j = 0; j < gridRow0.length; j++) {
			var	tempAcrossProduct = 1,
				tempDownProduct = 1,
				tempDiagRightProduct = 1,
				tempDiagLeftProduct = 1;
			...
			for (var k = 0; k < n; k++) {
				(j + k) < gridRow0.length ? tempAcrossProduct *= eval("gridRow" + i + "[j + k]") : tempAcrossProduct *= 1;
				(i + k) < initialArray.length ? tempDownProduct *= eval("gridRow" + (i + k) + "[j]") : tempDownProduct *= 1;
				(i + k) < initialArray.length && (j + k) < initialArray.length ? tempDiagRightProduct *= eval("gridRow" + (i + k) + "[j + k]") : tempDiagRightProduct *= 1;
				(i - k) >= 0 && (j - k) >= 0 ? tempDiagLeftProduct *= eval("gridRow" + (i - k) + "[j - k]") : tempDiagLeftProduct *= 1;
			}
			allProducts.push(tempAcrossProduct);
			allProducts.push(tempDownProduct);
			allProducts.push(tempDiagRightProduct);
...
function olderGreatestProductFromGrid () {
	var n = 4;
	outerLoop:
	for (var i = 0; i < initialArray.length; i++) {
		acrossProduct:
		for (var j = 0; j < gridRow0.length; j++) {
			tempProduct = 1;
			for (var k = 0; k < n; k++) {
				tempProduct *= eval("gridRow" + i + "[j + k]");
			}
			tempProduct > finalProduct ? finalProduct = tempProduct : n = 4;
		}
		downProduct:
		for (var j = 0; j < gridRow0.length; j++) {
			tempProduct = 1;
			for (var k = 0; k < n; k++) {
				((i + k) <= 19) ? tempProduct *= eval("gridRow" + (i + k) + "[j]") : tempProduct = 1;
			}
			tempProduct > finalProduct ? finalProduct = tempProduct : n = 4;
		}
		diagonalRightProduct:
		for (var j = 0; j < gridRow0.length; j++) {
			tempProduct = 1;
			for (var k = 0; k < n; k++) {
				((i + k) <= 19) ? tempProduct *= eval("gridRow" + (i + k) + "[j + k]") : tempProduct = 1;
			}
			tempProduct > finalProduct ? finalProduct = tempProduct : n = 4;
		}
		diagonalLeftProduct:
		for (var j = 0; j < gridRow0.length; j++) {
			tempProduct = 1;
			for (var k = 0; k < n; k++) {
				((i - k) >= 0) ? tempProduct *= eval("gridRow" + (i - k) + "[j - k]") : tempProduct = 1;
			}
			tempProduct > finalProduct ? finalProduct = tempProduct : n = 4;
		}
